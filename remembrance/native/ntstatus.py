NTSTATUS_CODES = [
    (0x0, 'STATUS_SUCCESS', 'The operation completed successfully'),
    (0x0, 'STATUS_WAIT_0',
     'The caller specified WaitAny for WaitType and one of the dispatcher objects in the Object array has been set to '
     'the signaled state'
     ),
    (0x1, 'STATUS_WAIT_1',
     'The caller specified WaitAny for WaitType and one of the dispatcher objects in the Object array has been set to '
    ...
    (0x106, 'STATUS_NOT_ALL_ASSIGNED',
     'Indicates not all privileges or groups that are referenced are assigned to the caller. This allows, '
     'for example, all privileges to be disabled without having to know exactly which privileges are assigned'
     ),
    ...
    (0x10a, 'STATUS_RXACT_COMMITTED',
     'This success level status indicates that the transaction state already exists for the registry subtree but that '
     'a transaction commit was previously aborted. The commit has now been completed'
     ),
    (0x10b, 'STATUS_NOTIFY_CLEANUP',
     'Indicates that a notify change request has been completed due to closing the handle that made the notify change '
     'request'
     ),
    ...
    (0x120, 'STATUS_TRANSLATION_COMPLETE',
     'The translator has translated these resources into the global space and no additional translations should be '
     'performed'
     ),
    (0x121, 'STATUS_DS_MEMBERSHIP_EVALUATED_LOCALLY',
     'The directory service evaluated group memberships locally, because it was unable to contact a global catalog '
     'server'
     ),
    ...
    (0xc0000721, 'STATUS_CALLBACK_RETURNED_THREAD_AFFINITY',
     'A threadpool worker thread entered a callback at thread affinity %p and exited at affinity %p. This is '
     'unexpected, indicating that the callback missed restoring the priority'
     ),
    ...
    (0x40000001, 'STATUS_THREAD_WAS_SUSPENDED',
     '{Thread Suspended} A thread termination occurred while the thread was suspended. The thread resumed, '
     'and termination proceeded'
     ),
    (0x40000002, 'STATUS_WORKING_SET_LIMIT_RANGE',
     '{Working Set Range Error} An attempt was made to set the working set minimum or maximum to values that are '
     'outside the allowable range'
     ),
    (0x40000003, 'STATUS_IMAGE_NOT_AT_BASE',
     '{Image Relocated} An image file could not be mapped at the address that is specified in the image file. Local '
     'fixes must be performed on this image'
     ),
    (0x40000004, 'STATUS_RXACT_STATE_CREATED',
     'This informational level status indicates that a specified registry subtree transaction state did not yet exist '
    ...
    (0x40000005, 'STATUS_SEGMENT_NOTIFICATION',
     ...
     'image. An exception is raised so that a debugger can load, unload, or track symbols and breakpoints within '
     'these 16-bit segments'
     ),
    (0x40000006, 'STATUS_LOCAL_USER_SESSION_KEY',
     '{Local Session Key} A user session key was requested for a local remote procedure call (RPC), connection. The '
     'session key that is returned is a constant value and not unique to this connection'
     ),
    (0x40000007, 'STATUS_BAD_CURRENT_DIRECTORY',
     ...
     'the current directory to %hs, or select CANCEL to exit'
     ),
    (0x40000008, 'STATUS_SERIAL_MORE_WRITES',
     '{Serial IOCTL Complete} A serial I/O operation was completed by another write to a serial port. (The '
     'IOCTL_SERIAL_XOFF_COUNTER reached zero.'
     ),
    (0x40000009, 'STATUS_REGISTRY_RECOVERED',
     '{Registry Recovery} One of the files that contains the system registry data had to be recovered by using a log '
    ...
    (0x4000000a, 'STATUS_FT_READ_RECOVERY_FROM_BACKUP',
     ...
     'successfully read the requested data from a redundant copy. This was done because the file system encountered a '
     'failure on a member of the fault-tolerant volume but was unable to reassign the failing area of the device'
     ),
    (0x4000000b, 'STATUS_FT_WRITE_RECOVERY',
     '{Redundant Write} To satisfy a write request, the Windows NT fault-tolerant file system successfully wrote a '
     'redundant copy of the information. This was done because the file system encountered a failure on a member of '
    ...
    (0x4000000c, 'STATUS_SERIAL_COUNTER_TIMEOUT',
     '{Serial IOCTL Timeout} A serial I/O operation completed because the time-out period expired. (The '
     'IOCTL_SERIAL_XOFF_COUNTER had not reached zero.'
     ),
    (0x4000000d, 'STATUS_NULL_LM_PASSWORD',
     '{Password Too Complex} The Windows password is too complex to be converted to a LAN Manager password. The LAN '
    ...
    (0x4000000f, 'STATUS_RECEIVE_PARTIAL',
     '{Partial Data Received} The network transport returned partial data to its client. The remaining data will be '
     'sent later'
     ),
    ...
    (0x40000029, 'STATUS_MP_PROCESSOR_MISMATCH',
     ...
     'the operating system restricts itself to the features of the least capable processor in the system. If problems '
    ...
    (0x4000002d, 'STATUS_DRIVERS_LEAKING_LOCKED_PAGES',
     'A device driver is leaking locked I/O pages and is causing system degradation. The system has automatically '
     'enabled the tracking code to try and catch the culprit'
     ),
    ...
    (0x401b00ec, 'STATUS_VIDEO_DRIVER_DEBUG_REPORT_REQUEST',
     ...
     'Some graphical operations may have failed. The next time you restart the machine, a dialog box appears, '
    ...
    (0x401e000a, 'STATUS_GRAPHICS_PARTIAL_DATA_POPULATED',
     "The specified buffer is not big enough to contain the entire requested dataset. Partial data is populated up to "
     "the size of the buffer. The caller needs to provide a buffer of the size as specified in the partially "
    ...
    (0x401e034c, 'STATUS_GRAPHICS_NO_MORE_ELEMENTS_IN_DATASET',
     'The specified dataset (for example, mode set, frequency range set, descriptor set, or topology), '
    ...
    (0x80000001, 'STATUS_GUARD_PAGE_VIOLATION',
     '{EXCEPTION} Guard Page Exception A page of memory that marks the end of a data structure, such as a stack or an '
    ...
    (0x8000000c, 'STATUS_GUID_SUBSTITUTION_MADE',
     '{GUID Substitution} During the translation of a globally unique identifier (GUID), to a Windows security ID ('
     'SID),, no administratively defined GUID prefix was found. A substitute prefix was used, which will not '
     'compromise system security. However, this may provide a more restrictive access than intended'
     ),
    ...
    (0x80000016, 'STATUS_VERIFY_REQUIRED',
     '{Verifying Disk} The media has changed and a verify operation is in progress; therefore, no reads or writes may '
     'be performed to the device, except those that are used in the verify operation'
     ),
    ...
    (0x80000018, 'STATUS_RXACT_COMMIT_NECESSARY',
     ...
     'but that a transaction commit was previously aborted. The commit has NOT been completed but has not been rolled '
     'back either; therefore, it may still be committed, if needed'
     ),
    ...
    (0x8000002a, 'STATUS_REGISTRY_HIVE_RECOVERED',
     '{Registry Hive Recovered} The registry hive (file),: %hs was corrupted and it has been recovered. Some data '
    ...
    (0x80000803, 'STATUS_DATA_LOST_REPAIR',
     'Windows discovered a corruption in the file %hs. This file has now been repaired. Check if any data in the file '
     'was lost because of the corruption'
     ),
    ...
    (0x801b00eb, 'STATUS_VIDEO_HUNG_DISPLAY_DRIVER_THREAD_RECOVERED',
     '{Display Driver Stopped Responding and recovered} The %hs display driver has stopped working normally. The '
     'recovery had been performed'
     ),
    ...
    (0xc0000004, 'STATUS_INFO_LENGTH_MISMATCH',
     'The specified information record length does not match the length that is required for the specified '
     'information class'
     ),
    ...
    (0xc0000006, 'STATUS_IN_PAGE_ERROR',
     'The instruction at 0x%08lx referenced memory at 0x%08lx. The required data was not placed into memory because '
     'of an I/O error status of 0x%08lx'
     ),
    ...
    (0xc000000c, 'STATUS_TIMER_NOT_CANCELED',
     'An attempt was made to cancel or set a timer that has an associated APC and the specified thread is not the '
    ...
    (0xc0000016, 'STATUS_MORE_PROCESSING_REQUIRED',
     '{Still Busy} The specified I/O request packet (IRP), cannot be disposed of because the I/O operation is not '
     'complete'
     ),
    (0xc0000017, 'STATUS_NO_MEMORY',
     '{Not Enough Quota} Not enough virtual memory or paging file quota is available to complete the specified '
     'operation'
     ),
    ...
    (0xc0000024, 'STATUS_OBJECT_TYPE_MISMATCH',
     '{Wrong Type} There is a mismatch between the type of object that is required by the requested operation and the '
    ...
    (0xc000002f, 'STATUS_PORT_MESSAGE_TOO_LONG',
     'The length of the message that was passed to NtRequestPort or NtRequestWaitReplyPort is longer than the maximum '
     'message that is allowed by the port'
     ),
    ...
    (0xc0000032, 'STATUS_DISK_CORRUPT_ERROR',
     '{Corrupt Disk} The file system structure on the disk is corrupt and unusable. Run the Chkdsk utility on the '
     'volume %hs'
     ),
    ...
    (0xc0000048, 'STATUS_PORT_ALREADY_SET',
     'An attempt was made to set the DebugPort or ExceptionPort of a process, but a port already exists in the '
     'process, or an attempt was made to set the CompletionPort of a file but a port was already set in the file, '
     'or an attempt was made to set the associated completion port of an ALPC port but it is already set'
     ),
    ...
    (0xc000004c, 'STATUS_BAD_WORKING_SET_LIMIT',
     'An attempt was made to set the working set limit to an invalid value (for example, the minimum greater than '
     'maximum),'
     ),
    (0xc000004d, 'STATUS_INCOMPATIBLE_FILE_MAP',
     'A section was created to map a file that is not compatible with an already existing section that maps the same '
     'file'
     ),
    ...
    (0xc0000057, 'STATUS_CTL_FILE_NOT_SUPPORTED',
     'An attempt was made to set the control attribute on a file. This attribute is not supported in the destination '
     'file system'
     ),
    (0xc0000058, 'STATUS_UNKNOWN_REVISION',
     'Indicates a revision number that was encountered or specified is not one that is known by the service. It may '
     'be a more recent revision than the service is aware of'
     ),
    ...
    (0xc0000069, 'STATUS_LAST_ADMIN',
     'Indicates the requested operation would disable or delete the last remaining administration account. This is '
     'not allowed to prevent creating a situation in which the system cannot be administrated'
     ),
    (0xc000006a, 'STATUS_WRONG_PASSWORD',
     'When trying to update a password, this return status indicates that the value provided as the current password '
     'is not correct'
     ),
    ...
    (0xc000006e, 'STATUS_ACCOUNT_RESTRICTION',
     'Indicates a referenced user name and authentication information are valid, but some user account restriction '
     'has prevented successful authentication (such as time-of-day restrictions),'
     ),
    ...
    (0xc000007b, 'STATUS_INVALID_IMAGE_FORMAT',
     '{Bad Image} %hs is either not designed to run on Windows or it contains an error. Try installing the program '
     'again using the original installation media or contact your system administrator or the software vendor for '
     'support'
     ),
    ...
    (0xc000007d, 'STATUS_BAD_INHERITANCE_ACL',
     ...
     'number of things. One of the more probable causes is the replacement of a CreatorId with a SID that did not fit '
     'into the ACE or ACL'
     ),
    ...
    (0xc0000099, 'STATUS_ALLOTTED_SPACE_EXCEEDED',
     ...
     'access control and primary group information, successive updates may exceed the amount of memory originally '
     'allotted. Because a quota may already have been charged to several processes that have handles to the object, '
     'it is not reasonable to alter the size of the allocated memory. Instead, a request that requires more memory '
     'than has been allotted must fail and the STATUS_ALLOTTED_SPACE_EXCEEDED error returned'
     ),
    ...
    (0xc00000a3, 'STATUS_DEVICE_NOT_READY',
     '{Drive Not Ready} The drive is not ready for use; its door may be open. Check drive %hs and make sure that a '
    ...
    (0xc00000a5, 'STATUS_BAD_IMPERSONATION_LEVEL',
     'A specified impersonation level is invalid. Also used to indicate that a required impersonation level was not '
     'provided'
     ),
    ...
    (0xc00000ae, 'STATUS_PIPE_BUSY',
     'The specified pipe is set to complete operations and there are current I/O operations queued so that it cannot '
     'be changed to queue operations'
     ),
    ...
    (0xc00000ba, 'STATUS_FILE_IS_A_DIRECTORY',
     'The file that was specified as a target is a directory, and the caller specified that it could be anything but '
     'a directory'
     ),
    ...
    (0xc00000cb, 'STATUS_BAD_DEVICE_TYPE',
     '{Incorrect Network Resource Type} The specified device type (LPT, for example), conflicts with the actual '
    ...
    (0xc00000d0, 'STATUS_REQUEST_NOT_ACCEPTED',
     'No more connections can be made to this remote computer at this time because the computer has already accepted '
    ...
    (0xc00000d4, 'STATUS_NOT_SAME_DEVICE',
     '{Incorrect Volume} The destination file of a rename request is located on a different device than the source of '
     'the rename request'
     ),
    ...
    (0xc00000d6, 'STATUS_VIRTUAL_CIRCUIT_CLOSED',
     '{Network Request Timeout} The session with a remote server has been disconnected because the time-out interval '
     'for a request has expired'
     ),
    (0xc00000d7, 'STATUS_NO_SECURITY_ON_OBJECT',
     'Indicates an attempt was made to operate on the security of an object that does not have security associated '
     'with it'
     ),
    ...
    (0xc00000e4, 'STATUS_INTERNAL_DB_CORRUPTION',
     'This error indicates that the requested operation cannot be completed due to a catastrophic media failure or an '
    ...
    (0xc00000e8, 'STATUS_INVALID_USER_BUFFER',
     'An access to a user buffer failed at an expected point in time. This code is defined because the caller does '
     'not want to accept STATUS_ACCESS_VIOLATION in its filter'
     ),
    (0xc00000e9, 'STATUS_UNEXPECTED_IO_ERROR',
     'If an I/O error that is not defined in the standard FsRtl filter is returned, it is converted to the following '
     'error, which is guaranteed to be in the filter. In this case, information is lost; however, the filter '
     'correctly handles the exception'
     ),
    (0xc00000ea, 'STATUS_UNEXPECTED_MM_CREATE_ERR',
     'If an MM error that is not defined in the standard FsRtl filter is returned, it is converted to one of the '
     'following errors, which are guaranteed to be in the filter. In this case, information is lost; however, '
    ...
    (0xc00000ee, 'STATUS_LOGON_SESSION_EXISTS',
     'An attempt has been made to start a new session manager or LSA logon session by using an ID that is already in '
     'use'
     ),
    ...
    (0xc0000107, 'STATUS_FILES_OPEN',
     'The user attempted to force close the files on a redirected drive, but there were opened files on the drive, '
     'and the user did not specify a sufficient level of force'
     ),
    (0xc0000108, 'STATUS_CONNECTION_IN_USE',
     'The user attempted to force close the files on a redirected drive, but there were opened directories on the '
    ...
    (0xc000010c, 'STATUS_NO_GUID_TRANSLATION',
     ...
     'in the security descriptor could not be translated into a GUID that could be stored by the file system. This '
    ...
    (0xc000011c, 'STATUS_RXACT_INVALID_STATE',
     'Indicates that the transaction state of a registry subtree is incompatible with the requested operation. For '
    ...
    (0xc000011d, 'STATUS_RXACT_COMMIT_FAILURE',
     'Indicates an error has occurred during a registry transaction commit. The database has been left in an unknown, '
     'but probably inconsistent, state. The state of the registry transaction is left as COMMITTING'
     ),
    (0xc000011e, 'STATUS_MAPPED_FILE_SIZE_ZERO',
     'An attempt was made to map a file of size zero with the maximum size specified as zero'
     ),
    (0xc000011f, 'STATUS_TOO_MANY_OPENED_FILES',
     'Too many files are opened on a remote server. This error should only be returned by the Windows redirector on a '
     'remote drive'
     ),
    ...
    (0xc0000123, 'STATUS_FILE_DELETED',
     'An I/O request other than close was performed on a file after it was deleted, which can only happen to a '
     'request that did not complete before the last handle was closed via NtClose'
     ),
    (0xc0000124, 'STATUS_SPECIAL_ACCOUNT',
     'Indicates an operation that is incompatible with built-in accounts has been attempted on a built-in (special), '
    ...
    (0xc0000128, 'STATUS_FILE_CLOSED',
     'An I/O request other than close and several other special case operations was attempted using a file object '
    ...
    (0xc000012a, 'STATUS_THREAD_NOT_IN_PROCESS',
     'An attempt was made to operate on a thread within a specific process, but the specified thread is not in the '
     'specified process'
     ),
    (0xc000012b, 'STATUS_TOKEN_ALREADY_IN_USE',
     'An attempt was made to establish a token for use as a primary token but the token is already in use. A token '
     'can only be the primary token of one process at a time'
     ),
    (0xc000012c, 'STATUS_PAGEFILE_QUOTA_EXCEEDED',
     'The page file quota was exceeded'),
    (0xc000012d, 'STATUS_COMMITMENT_LIMIT',
     '{Out of Virtual Memory} Your system is low on virtual memory. To ensure that Windows runs correctly, '
     'increase the size of your virtual memory paging file. For more information, see Help'
     ),
    ...
    (0xc0000133, 'STATUS_TIME_DIFFERENCE_AT_DC',
     'The time at the primary domain controller is different from the time at the backup domain controller or member '
     'server by too large an amount'
     ),
    (0xc0000134, 'STATUS_SYNCHRONIZATION_REQUIRED',
     'The SAM database on a Windows Server operating system is significantly out of synchronization with the copy on '
     'the domain controller. A complete synchronization is required'
     ),
    (0xc0000135, 'STATUS_DLL_NOT_FOUND',
     '{Unable To Locate Component} This application has failed to start because %hs was not found. Reinstalling the '
    ...
    (0xc000013b, 'STATUS_LOCAL_DISCONNECT',
     '{Virtual Circuit Closed} The network transport on your computer has closed a network connection. There may or '
     'may not be I/O requests outstanding'
     ),
    (0xc000013c, 'STATUS_REMOTE_DISCONNECT',
     '{Virtual Circuit Closed} The network transport on a remote computer has closed a network connection. There may '
     'or may not be I/O requests outstanding'
     ),
    (0xc000013d, 'STATUS_REMOTE_RESOURCES',
     '{Insufficient Resources on Remote Computer} The remote computer has insufficient resources to complete the '
     'network request. For example, the remote computer may not have enough available memory to carry out the request '
     'at this time'
     ),
    (0xc000013e, 'STATUS_LINK_FAILED',
     ...
     'There is probably something wrong with the network software protocol or the network hardware on the remote '
     'computer'
     ),
    (0xc000013f, 'STATUS_LINK_TIMEOUT',
     '{Virtual Circuit Closed} The network transport on your computer has closed a network connection because it had '
     'to wait too long for a response from the remote computer'
     ),
    ...
    (0xc0000142, 'STATUS_DLL_INIT_FAILED',
     '{DLL Initialization Failed} Initialization of the dynamic link library %hs failed. The process is terminating '
     'abnormally'
     ),
    ...
    (0xc0000149, 'STATUS_WRONG_PASSWORD_CORE',
     '{Incorrect Password to LAN Manager Server} You specified an incorrect password to a LAN Manager 2.x or MS-NET '
     'server'
     ),
    (0xc000014a, 'STATUS_ILLEGAL_FLOAT_CONTEXT',
     '{EXCEPTION} A real-mode application issued a floating-point instruction and floating-point hardware is not '
     'present'
     ),
    ...
    (0xc000014c, 'STATUS_REGISTRY_CORRUPT',
     '{The Registry Is Corrupt} The structure of one of the files that contains registry data is corrupt; the image '
     'of the file in memory is corrupt; or the file could not be recovered because the alternate copy or log was '
     'absent or corrupt'
     ),
    (0xc000014d, 'STATUS_REGISTRY_IO_FAILED',
     "An I/O operation initiated by the Registry failed and cannot be recovered. The registry could not read in, "
    ...
    (0xc000014f, 'STATUS_UNRECOGNIZED_VOLUME',
     'The volume does not contain a recognized file system. Be sure that all required file system drivers are loaded '
    ...
    (0xc0000156, 'STATUS_TOO_MANY_SECRETS',
     'The maximum number of secrets that may be stored in a single system was exceeded. The length and number of '
     'secrets is limited to satisfy U.S. State Department export restrictions'
     ),
    (0xc0000157, 'STATUS_SECRET_TOO_LONG',
     'The length of a secret exceeds the maximum allowable length. The length and number of secrets is limited to '
    ...
    (0xc000015b, 'STATUS_LOGON_TYPE_NOT_GRANTED',
     'A user has requested a type of logon (for example, interactive or network), that has not been granted. An '
     'administrator has control over who may logon interactively and through the network'
     ),
    (0xc000015c, 'STATUS_NOT_REGISTRY_FILE',
     'The system has attempted to load or restore a file into the registry, and the specified file is not in the '
    ...
    (0xc0000160, 'STATUS_ILL_FORMED_SERVICE_ENTRY',
     'A configuration registry node that represents a driver service entry was ill-formed and did not contain the '
     'required value entries'
     ),
    (0xc0000161, 'STATUS_ILLEGAL_CHARACTER',
     'An illegal character was encountered. For a multibyte character set, this includes a lead byte without a '
    ...
    (0xc0000166, 'STATUS_FLOPPY_WRONG_CYLINDER',
     '{Floppy Disk Error} While accessing a floppy disk, the track address from the sector ID field was found to be '
     'different from the track address that is maintained by the controller'
     ),
    ...
    (0xc000016c, 'STATUS_SHARED_IRQ_BUSY',
     'An attempt was made to open a device that was sharing an interrupt request (IRQ), with other devices. At least '
     'one other device that uses that IRQ was already opened. Two concurrent opens of devices that share an IRQ and '
     'only work via interrupts is not supported for the particular bus type that the devices use'
     ),
    ...
    (0xc000016e, 'STATUS_BIOS_FAILED_TO_CONNECT_INTERRUPT',
     'The basic input/output system (BIOS), failed to connect a system interrupt to the device or bus for which the '
     'device is connected'
     ),
    ...
    (0xc0000198, 'STATUS_NOLOGON_INTERDOMAIN_TRUST_ACCOUNT',
     'The account used is an interdomain trust account. Use your global user account or local user account to access '
     'this server'
     ),
    ...
    (0xc0000219, 'STATUS_DEBUG_ATTACH_FAILED',
     '{Unexpected Failure in DebugActiveProcess} An unexpected failure occurred while processing a DebugActiveProcess '
     'API request. You may choose OK to terminate the process, or Cancel to ignore the error'
     ),
    ...
    (0xc000021f, 'STATUS_REPLY_MESSAGE_MISMATCH',
     '{Reply Message Mismatch} An attempt was made to reply to an LPC message, but the thread specified by the client '
     'ID in the message was not waiting on that message'
     ),
    ...
    (0xc0000222, 'STATUS_LOST_WRITEBEHIND_DATA',
     '{Delayed Write Failed} Windows was unable to save all the data for the file %hs. The data has been lost. This '
     'error may be caused by a failure of your computer hardware or network connection. Try to save this file elsewhere'
     ),
    (0xc0000223, 'STATUS_CLIENT_SERVER_PARAMETERS_INVALID',
     'The parameters passed to the server in the client/server shared memory window were invalid. Too much data may '
    ...
    (0xc000022e, 'STATUS_FOUND_OUT_OF_SCOPE',
     'The attempt to find the object found an object on the volume that matches by ID; however, it is out of the '
     'scope of the handle that is used for the operation'
     ),
    ...
    (0xc0000249, 'STATUS_IMAGE_MP_UP_MISMATCH',
     '{UP/MP Image Mismatch} The image %hs has been modified for use on a uniprocessor system, but you are running it '
     'on a multiprocessor machine. Reinstall the image file'
     ),
    (0xc0000250, 'STATUS_INSUFFICIENT_LOGON_INFO',
     'There is insufficient account information to log you on'),
    (0xc0000251, 'STATUS_BAD_DLL_ENTRYPOINT',
     '{Invalid DLL Entrypoint} The dynamic link library %hs is not written correctly. The stack pointer has been left '
     'in an inconsistent state. The entry point should be declared as WINAPI or STDCALL. Select YES to fail the DLL '
     'load. Select NO to continue execution. Selecting NO may cause the application to operate incorrectly'
     ),
    (0xc0000252, 'STATUS_BAD_SERVICE_ENTRYPOINT',
     '{Invalid Service Callback Entrypoint} The %hs service is not written correctly. The stack pointer has been left '
     'in an inconsistent state. The callback entry point should be declared as WINAPI or STDCALL. Selecting OK will '
    ...
    (0xc0000256, 'STATUS_REGISTRY_QUOTA_LIMIT',
     '{Low On Registry Space} The system has reached the maximum size that is allowed for the system part of the '
    ...
    (0xc0000259, 'STATUS_LICENSE_QUOTA_EXCEEDED',
     ...
     'to the service at this time because the service has already accepted the maximum number of connections'
     ),
    ...
    (0xc000025b, 'STATUS_PWD_TOO_RECENT',
     ...
     'users from changing back to a familiar, but potentially discovered, password. If you feel your password has '
     'been compromised, contact your administrator immediately to have a new one assigned'
     ),
    (0xc000025c, 'STATUS_PWD_HISTORY_CONFLICT',
     ...
     'account does not allow this. Select a password that you have not previously used'
     ),
    ...
    (0xc0000264, 'STATUS_RESOURCE_NOT_OWNED',
     '{Application Error} The application attempted to release a resource it did not own. Click OK to terminate the '
     'application'
     ),
    ...
    (0xc0000268, 'STATUS_EVALUATION_EXPIRATION',
     ...
     'system will shutdown in 1 hour. To restore access to this installation of Windows, upgrade this installation by '
    ...
    (0xc0000269, 'STATUS_ILLEGAL_DLL_RELOCATION',
     ...
     'properly. The relocation occurred because the DLL %hs occupied an address range that is reserved for Windows '
     'system DLLs. The vendor supplying the DLL should be contacted for a new DLL'
     ),
    (0xc000026a, 'STATUS_LICENSE_VIOLATION',
     '{License Violation} The system has detected tampering with your registered product type. This is a violation of '
     'your software license. Tampering with the product type is not permitted'
     ),
    ...
    (0xc00002ad, 'STATUS_DS_CANT_ON_RDN',
     'The directory service cannot perform the requested operation on the Relatively Defined Name (RDN), attribute of '
     'an object'
     ),
    ...
    (0xc00002c2, 'STATUS_DRIVER_FAILED_SLEEP',
     '{System Standby Failed} The driver %hs does not support standby mode. Updating this driver may allow the system '
    ...
    (0xc00002cb, 'STATUS_DS_SAM_INIT_FAILURE',
     'Security Accounts Manager initialization failed because of the following error: %hs Error Status: 0x%x. Click '
     'OK to shut down this system and restart in Directory Services Restore Mode. Check the event log for more '
    ...
    (0xc00002e2, 'STATUS_DS_INIT_FAILURE',
     'The directory service could not start because of the following error: %hs Error Status: 0x%x. Click OK to shut '
     'down this system and restart in Directory Services Restore Mode. Check the event log for more detailed '
     'information'
     ),
    ...
    (0xc00002e7, 'STATUS_DS_MACHINE_ACCOUNT_QUOTA_EXCEEDED',
     'Your computer could not be joined to the domain. You have exceeded the maximum number of computer accounts you '
     'are allowed to create in this domain. Contact your system administrator to have this limit reset or increased'
     ),
    ...
    (0xc00002ed, 'STATUS_DS_SAM_INIT_FAILURE_CONSOLE',
     'Security Accounts Manager initialization failed because of the following error: %hs Error Status: 0x%x. Click '
     'OK to shut down the system. You can use the recovery console to diagnose the system further'
     ),
    ...
    (0xc0000300, 'STATUS_NOT_SUPPORTED_ON_SBS',
     'This operation is not supported on a computer running Windows Server 2003 operating system for Small Business '
     'Server'
     ),
    ...
    (0xc0000320, 'STATUS_PKINIT_FAILURE',
     'The Kerberos protocol encountered an error while validating the KDC certificate during smart card logon. There '
    ...
    (0xc0000352, 'STATUS_EFS_ALG_BLOB_TOO_BIG',
     'The encryption algorithm that is used on the source file needs a bigger key buffer than the one that is used on '
    ...
    (0xc0000355, 'STATUS_DS_VERSION_CHECK_FAILURE',
     'This version of Windows is not compatible with the behavior version of the directory forest, domain, '
    ...
    (0xc0000366, 'STATUS_DEVICE_ENUMERATION_ERROR',
     'The device encountered an error while applying power or reading the device configuration. This may be caused by '
     'a failure of your hardware or by a poor connection'
     ),
    (0xc0000368, 'STATUS_MOUNT_POINT_NOT_RESOLVED',
     'The create operation failed because the name contained at least one mount point that resolves to a volume to '
    ...
    (0xc0000388, 'STATUS_DOWNGRADE_DETECTED',
     'The system detected a possible attempt to compromise security. Ensure that you can contact the server that '
     'authenticated you'
     ),
    (0xc0000389, 'STATUS_SMARTCARD_CERT_REVOKED',
     'The smart card certificate used for authentication has been revoked. Contact your system administrator. There '
     'may be additional information in the event log'
     ),
    (0xc000038a, 'STATUS_ISSUING_CA_UNTRUSTED',
     'An untrusted certificate authority was detected while processing the smart card certificate that is used for '
    ...
    (0xc000040b, 'STATUS_CROSSREALM_DELEGATION_FAILURE',
     'An attempt was made by this server to make a Kerberos constrained delegation request for a target that is '
     'outside the server realm. This action is not supported and the resulting error indicates a misconfiguration on '
     'the allowed-to-delegate-to list for this server. Contact your administrator'
     ),
    (0xc000040c, 'STATUS_REVOCATION_OFFLINE_KDC',
     'The revocation status of the domain controller certificate used for smart card authentication could not be '
     'determined. There is additional information in the system event log. Contact your system administrator'
     ),
    (0xc000040d, 'STATUS_ISSUING_CA_UNTRUSTED_KDC',
     ...
     'authentication. There is additional information in the system event log. Contact your system administrator'
     ),
    (0xc000040e, 'STATUS_KDC_CERT_EXPIRED',
     'The domain controller certificate used for smart card logon has expired. Contact your system administrator with '
     'the contents of your system event log'
     ),
    (0xc000040f, 'STATUS_KDC_CERT_REVOKED',
     'The domain controller certificate used for smart card logon has been revoked. Contact your system administrator '
    ...
    (0xc0000413, 'STATUS_AUTHENTICATION_FIREWALL_FAILED',
     'Logon Failure: The machine you are logging onto is protected by an authentication firewall. The specified '
    ...
    (0xc0000415, 'STATUS_HUNG_DISPLAY_DRIVER_THREAD',
     '{Display Driver Stopped Responding} The %hs display driver has stopped working normally. Save your work and '
     'reboot the system to restore full display functionality. The next time you reboot the machine a dialog will be '
     'displayed giving you a chance to report this failure to Microsoft'
     ),
    (0xc0000416,
    ...
    (0xc0000424, 'STATUS_INCOMPATIBLE_DRIVER_BLOCKED',
     '%2 has been blocked from loading due to incompatibility with this system. Contact your software vendor for a '
    ...
    (0xc0000428, 'STATUS_INVALID_IMAGE_HASH',
     'The hash for image %hs cannot be found in the system catalogs. The image is likely corrupt or the victim of '
     'tampering'
     ),
    ...
    (0xc0000451, 'STATUS_AMBIGUOUS_SYSTEM_DEVICE',
     'The requested system device cannot be identified due to multiple indistinguishable devices potentially matching '
    ...
    (0xc0000711, 'STATUS_APC_RETURNED_WHILE_IMPERSONATING',
     'A thread pool worker thread is impersonating a client, after executing an APC. This is unexpected, indicating '
    ...
    (0xc000071b, 'STATUS_CALLBACK_RETURNED_THREAD_PRIORITY',
     'A threadpool worker thread entered a callback at thread base priority 0x%x and exited at priority 0x%x. This is '
     'unexpected, indicating that the callback missed restoring the priority'
     ),
    ...
    (0xc0000905, 'STATUS_FORMS_AUTH_REQUIRED',
     'Access Denied. Before opening files in this location, you must first browse to the e.g. site and select the '
     'option to log on automatically'
     ),
    ...
    (0xc000a080, 'STATUS_LOST_WRITEBEHIND_DATA_NETWORK_DISCONNECTED',
     '{Delayed Write Failed} Windows was unable to save all the data for the file %hs; the data has been lost. This '
     'error may be caused by network connectivity issues. Try to save this file elsewhere'
     ),
    (0xc000a081, 'STATUS_LOST_WRITEBEHIND_DATA_NETWORK_SERVER_ERROR',
     '{Delayed Write Failed} Windows was unable to save all the data for the file %hs; the data has been lost. This '
     'error was returned by the server on which the file exists. Try to save this file elsewhere'
     ),
    (0xc000a082, 'STATUS_LOST_WRITEBEHIND_DATA_LOCAL_DISK_ERROR',
     '{Delayed Write Failed} Windows was unable to save all the data for the file %hs; the data has been lost. This '
     'error may be caused if the device has been removed or the media is write-protected'
     ),
    ...
    (0xc000a2a3, 'STATUS_OFFLOAD_READ_FILE_NOT_SUPPORTED',
     'Offload read operations cannot be performed on: Compressed files Sparse files Encrypted files File system '
     'metadata file'
     ),
    (0xc000a2a4, 'STATUS_OFFLOAD_WRITE_FILE_NOT_SUPPORTED',
     'Offload write operations cannot be performed on: Compressed files Sparse files Encrypted files File system '
     'metadata file'
     ),
    ...
    (0xc0040035, 'STATUS_PNP_BAD_MPS_TABLE',
     'A device is missing in the system BIOS MPS table. This device will not be used. Contact your system vendor for '
    ...
    (0xc0040039, 'STATUS_IO_REISSUE_AS_CACHED',
     'Reissue the given operation as a cached I/O operatio'),
    ...
    (0xc00a000b, 'STATUS_CTX_MODEM_RESPONSE_TIMEOUT',
     'The modem did not respond to the command sent to it. Verify that the modem cable is properly attached and the '
     'modem is turned on'
     ),
    ...
    (0xc00a000d, 'STATUS_CTX_MODEM_RESPONSE_NO_DIALTONE',
     'A dial tone was not detected within the required time. Verify that the phone cable is properly attached and '
     'functional'
     ),
    ...
    (0xc00a0017, 'STATUS_CTX_WINSTATION_BUSY',
     'The requested operation cannot be completed because the terminal connection is currently processing a connect, '
    ...
    (0xc00a0030, 'STATUS_CTX_SHADOW_INVALID',
     'The requested session cannot be controlled remotely. You cannot control your own session, a session that is '
     'trying to control your session, a session that has no user logged on, or other sessions from the console'
     ),
    ...
    (0xc00a0033, 'STATUS_CTX_CLIENT_LICENSE_NOT_SET',
     ...
     'has not been entered for this copy of the terminal client. Contact your system administrator for help in '
     'entering a valid, unique license number for this terminal server client. Click OK to continue'
     ),
    (0xc00a0034, 'STATUS_CTX_CLIENT_LICENSE_IN_USE',
     'Your request to connect to this terminal server has been rejected. Your terminal server client license number '
     'is currently being used by another user. Contact your system administrator to obtain a new copy of the terminal '
     'server client with a valid, unique license number. Click OK to continue'
     ),
    ...
    (0xc0150002, 'STATUS_SXS_CANT_GEN_ACTCTX',
     'Windows was unble to process the application binding information. Refer to the system event log for further '
     'information'
     ),
    ...
    (0xc0190005, 'STATUS_RM_NOT_ACTIVE',
     'Transaction support within the specified file system resource manager was not started or was shut down due to '
     'an error'
     ),
    ...
    (0xc0190014, 'STATUS_TRANSACTION_NOT_REQUESTED',
     'The caller has called a response API, but the response is not expected because the transaction manager did not '
    ...
    (0xc0190021, 'STATUS_OBJECT_NO_LONGER_EXISTS',
     'The object (file, stream, or link), that corresponds to the handle has been deleted by a transaction savepoint '
     'rollback'
     ),
    ...
    (0xc0190023, 'STATUS_STREAM_MINIVERSION_NOT_VALID',
     'The specified file miniversion was found but has been invalidated. The most likely cause is a transaction '
     'savepoint rollback'
     ),
    (0xc0190024,
    ...
    (0xc0190036, 'STATUS_FILE_IDENTITY_NOT_PERSISTENT',
     'The file cannot be opened in a transaction because its identity depends on the outcome of an unresolved '
     'transaction'
     ),
    ...
    (0xc019004a, 'STATUS_TM_IDENTITY_MISMATCH',
     'The call to create a transaction manager object failed because the Tm Identity that is stored in the log file '
    ...
    (0xc019004c, 'STATUS_CANNOT_ACCEPT_TRANSACTED_WORK',
     'The transactional resource manager cannot currently accept transacted work due to a transient condition, '
     'such as low resources'
     ),
    (0xc019004d, 'STATUS_CANNOT_ABORT_TRANSACTIONS',
     'The transactional resource manager had too many transactions outstanding that could not be aborted. The '
     'transactional resource manager has been shut down'
     ),
    ...
    (0xc0190052, 'STATUS_TRANSACTIONMANAGER_NOT_ONLINE',
     'The specified resource manager was unable to create an enlistment because its associated transaction manager is '
     'not online'
     ),
    (0xc0190053, 'STATUS_TRANSACTIONMANAGER_RECOVERY_NAME_COLLISION',
     'The specified transaction manager was unable to create the objects contained in its log file in the Ob '
     'namespace. Therefore, the transaction manager was unable to recover'
     ),
    ...
    (0xc0190058, 'STATUS_TRANSACTION_RECORD_TOO_LONG',
     'The specified operation could not be performed because the record to be logged was too long. This can occur '
     'because either there are too many enlistments on this transaction or the combined RecoveryInformation being '
    ...
    (0xc01b00ea, 'STATUS_VIDEO_HUNG_DISPLAY_DRIVER_THREAD',
     '{Display Driver Stopped Responding} The %hs display driver has stopped working normally. Save your work and '
     'reboot the system to restore full display functionality. The next time you reboot the computer, a dialog box '
    ...
    (0xc01c0006, 'STATUS_FLT_NOT_SAFE_TO_POST_OPERATION',
     'Posting this operation to a worker thread for further processing is not safe at this time because it could lead '
    ...
    (0xc01c0008, 'STATUS_FLT_FILTER_NOT_READY',
     'The filter is not ready for attachment to volumes because it has not finished initializing (FltStartFiltering '
     'has not been called),'
     ),
    (0xc01c0009, 'STATUS_FLT_POST_OPERATION_CLEANUP',
     'The filter must clean up any operation-specific context at this time because it is being removed from the '
     'system before the operation is completed by the lower drivers'
     ),
    ...
    (0xc01c000b, 'STATUS_FLT_DELETING_OBJECT',
     'The object specified for this action is in the process of being deleted; therefore, the action requested cannot '
    ...
    (0xc01d0006, 'STATUS_MONITOR_INVALID_SERIAL_NUMBER_MONDSC_BLOCK',
     "The provided monitor descriptor block is either corrupted or does not contain the monitor's detailed serial "
     "number"
     ),
    ...
    (0xc01e000c, 'STATUS_GRAPHICS_PRESENT_UNOCCLUDED',
     'Previous exclusive VidPn source owner has released its ownershi'
     ),
    ...
    (0xc01e0302,
     ...
     'The specified VidPN topology is valid but is not currently supported by the display adapter due to allocation '
     'of its resources'
     ),
    ...
    (0xc01e0306, 'STATUS_GRAPHICS_VIDPN_MODALITY_NOT_SUPPORTED',
     'The specified VidPN modality is not supported (for example, at least two of the pinned modes are not '
     'co-functional),'
     ),
    ...
    (0xc01e0323, 'STATUS_GRAPHICS_NO_RECOMMENDED_FUNCTIONAL_VIDPN',
     'The miniport does not have a recommendation regarding the request to provide a functional VidPN given the '
    ...
    (0xc01e0325,
     ...
     'The system failed to determine a mode that is supported by both the display adapter and the monitor connected '
     'to it'
     ),
    (0xc01e0326,
    ...
    (0xc01e035c, 'STATUS_GRAPHICS_CLIENTVIDPN_NOT_SET',
     'The client VidPN is not set on this adapter (for example, no user mode-initiated mode changes have taken place '
     'on this adapter),'
     ),
    ...
    (0xc01e050f, 'STATUS_GRAPHICS_OPM_SPANNING_MODE_ENABLED',
     'DxgkDdiOpmCreateProtectedOutput(), could not create a protected output because the video present yarget is in '
     'spanning mode'
     ),
    (0xc01e0510, 'STATUS_GRAPHICS_OPM_THEATER_MODE_ENABLED',
     'DxgkDdiOpmCreateProtectedOutput(), could not create a protected output because the video present target is in '
     'theater mode'
     ),
    (0xc01e0511, 'STATUS_GRAPHICS_PVP_HFS_FAILED',
     "The function call failed because the display adapter's hardware functionality scan (HFS), failed to validate "
    ...
    (0xc01e0516, 'STATUS_GRAPHICS_OPM_HDCP_SRM_NEVER_SET',
     'DxgkDdiOPMGetInformation(), cannot return the version of the SRM being used because the application never '
     'successfully passed an SRM to the protected output'
     ),
    (0xc01e0517, 'STATUS_GRAPHICS_OPM_RESOLUTION_TOO_HIGH',
     "DxgkDdiOPMConfigureProtectedOutput(), cannot enable the specified output protection technology because the "
     "output's screen resolution is too high"
     ),
    (0xc01e0518, 'STATUS_GRAPHICS_OPM_ALL_HDCP_HARDWARE_ALREADY_IN_USE'
     ,
     "DxgkDdiOPMConfigureProtectedOutput(), cannot enable HDCP because other physical outputs are using the display "
     "adapter's HDCP hardware"
     ),
    (0xc01e051a, 'STATUS_GRAPHICS_OPM_PROTECTED_OUTPUT_NO_LONGER_EXISTS'
     ,
     "The operating system asynchronously destroyed this OPM-protected output because the operating system state "
     "changed. This error typically occurs because the monitor PDO associated with this protected output was removed "
     "or stopped, the protected output's session became a nonconsole session, or the protected output's desktop "
     "became inactive"
     ),
    (0xc01e051b, 'STATUS_GRAPHICS_OPM_SESSION_TYPE_CHANGE_IN_PROGRESS',
     'OPM functions cannot be called when a session is changing its type. Three types of sessions currently exist: '
     'console, disconnected, and remote (RDP or ICA),'
     ),
    (0xc01e051c,
    ...
    (0xc01e051f,
     ...
     ,
     'The DxgkDdiOPMGetCOPPCompatibleInformation, DxgkDdiOPMGetInformation, or DxgkDdiOPMConfigureProtectedOutput '
     'function failed. This error is returned only if a protected output has COPP semantics. '
     'DxgkDdiOPMGetCOPPCompatibleInformation returns this error code if the caller requested OPM-specific '
     'information. DxgkDdiOPMGetInformation always returns this error if a protected output has COPP semantics. '
    ...
    (0xc01e0520, 'STATUS_GRAPHICS_OPM_SIGNALING_NOT_SUPPORTED',
     'The DxgkDdiOPMGetCOPPCompatibleInformation and DxgkDdiOPMConfigureProtectedOutput functions return this error '
     'if the display driver does not support the DXGKMDT_OPM_GET_ACP_AND_CGMSA_SIGNALING and '
     'DXGKMDT_OPM_SET_ACP_AND_CGMSA_SIGNALING GUIDs'
     ),
    (0xc01e0521, 'STATUS_GRAPHICS_OPM_INVALID_CONFIGURATION_REQUEST',
     'The DxgkDdiOPMConfigureProtectedOutput function returns this error code if the passed-in sequence number is not '
    ...
    (0xc01e0586,
     ...
     'the DDC/CI get timing report and timing message command to get a timing report from a monitor'
     ),
    (0xc01e0587, 'STATUS_GRAPHICS_DDCCI_INVALID_CAPABILITIES_STRING',
     'A monitor returned a DDC/CI capabilities string that did not comply with the ACCESS.bus 3.0, DDC/CI 1.1, '
     'or MCCS 2 Revision 1 specification'
     ),
    ...
    (0xc01e058b, 'STATUS_GRAPHICS_DDCCI_INVALID_MESSAGE_CHECKSUM',
     "This error occurred because the value in a DDC/CI message's checksum field did not match the message's computed "
     "checksum value. This error implies that the data was corrupted while it was being transmitted from a monitor to "
     "a computer"
     ),
    ...
    (0xc01e058d, 'STATUS_GRAPHICS_MONITOR_NO_LONGER_EXISTS',
     "The operating system asynchronously destroyed the monitor that corresponds to this handle because the operating "
     "system's state changed. This error typically occurs because the monitor PDO associated with this handle was "
     "removed or stopped, or a display mode change occurred. A display mode change occurs when Windows sends a "
     "WM_DISPLAYCHANGE message to applications"
     ),
    (0xc01e05e0, 'STATUS_GRAPHICS_ONLY_CONSOLE_SESSION_SUPPORTED',
     'This function can be used only if a program is running in the local console session. It cannot be used if a '
     'program is running on a remote desktop session or on a terminal server session'
     ),
    ...
    (0xc01e05e3, 'STATUS_GRAPHICS_MIRRORING_DEVICES_NOT_SUPPORTED',
     'This function does not support GDI mirroring display devices because GDI mirroring display devices do not have '
     'any physical monitors associated with them'
     ),
    (0xc01e05e4, 'STATUS_GRAPHICS_INVALID_POINTER',
     'The function failed because an invalid pointer parameter was passed to it. A pointer parameter is invalid if it '
     'is null, is not correctly aligned, or points to an invalid address or to a kernel mode address'
     ),
    (0xc01e05e5,
    ...
    (0xc01e05e8, 'STATUS_GRAPHICS_SESSION_TYPE_CHANGE_IN_PROGRESS',
     ...
     'current session is changing its type. Three types of sessions currently exist: console, disconnected, '
    ...
    (0xc0210018, 'STATUS_FVE_TPM_INVALID_PCR',
     'The system boot information changed or the TPM locked out access to BitLocker encryption keys until the '
    ...
    (0xc0210028, 'STATUS_FVE_CONV_RECOVERY_FAILED',
     'Bitlocker Drive Encryption failed to recover from aborted conversion. This could be due to either all '
     'conversion logs being corrupted or the media being write-protected'
     ),
    ...
    (0xc0232000, 'STATUS_NDIS_DOT11_AUTO_CONFIG_ENABLED',
     'The wireless LAN interface is in auto-configuration mode and does not support the requested parameter change '
     'operation'
     ),
    ...
    (0xc0368006, 'STATUS_IPSEC_DOSP_MAX_PER_IP_RATELIMIT_QUEUES',
     'IPsec Dos Protection failed to create per internal IP ratelimit queue because there is already maximum number '
     'of queues allowed by policy'
     ),
    ...
    (0xc03a0017, 'STATUS_VHD_CHILD_PARENT_SIZE_MISMATCH',
     'The chain of virtual hard disks is corrupted. There is a mismatch in the virtual sizes of the parent virtual '
     'hard disk and differencing disk'
     ),
